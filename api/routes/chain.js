...
const ccpPath = path.resolve(__dirname, '../../chain', 'basic-network', 'connection.json');
...
async function add(user, content) {
  ...
  try {
    ...
    const walletPath = path.join(process.cwd(), '../chain/docrec/wallet');
...
/**
 * @api {get} /chain/validate Validate non-repudable content
 * @apiName NR
 * @apiGroup Validate
 *
 * @apiParam {body} NR payload.
